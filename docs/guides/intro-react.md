...
Например, вы можете вложить [`Text`](text) и [`TextInput`](textinput) внутри [`View`](view) ниже, и React Native отобразит их вместе:
...
Вы можете поместить в кафе столько кошек, сколько захотите. Каждая `<Кошка>` отображает уникальный элемент, который вы можете настроить с помощью реквизитов.
...
Большинство основных компонентов React Native также могут быть настроены с помощью реквизитов. Например, при использовании [`Image`](image.md) вы передаете ему реквизит [`source`](image.md), чтобы определить, какое изображение он показывает:
...
У `Image` есть [множество различных реквизитов](image.md), включая [`style`](image.md), который принимает JS-объект пар свойств-значений, связанных с дизайном и компоновкой.
...
Вы можете создать множество вещей с помощью реквизитов и основных компонентов [`Text`](text.md), [`Image`](image.md) и [`View`](view.md)! Но чтобы создать что-то интерактивное, вам понадобится состояние.
