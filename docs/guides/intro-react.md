...
Вы можете поместить в кафе столько кошек, сколько захотите. Каждая `<Cat>` отображает уникальный элемент, который вы можете настроить с помощью реквизитов.
...
Большинство основных компонентов React Native также могут быть настроены с помощью реквизитов. Например, при использовании [`Image`](../components/image.md) вы передаете ему реквизит [`source`](../components/image.md), чтобы определить, какое изображение он показывает:
...
У `Image` есть [множество различных реквизитов](../components/image.md), включая [`style`](../components/image.md), который принимает JS-объект пар свойств-значений, связанных с дизайном и компоновкой.
