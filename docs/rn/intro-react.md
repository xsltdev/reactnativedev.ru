...
Например, вы можете вложить [`Text`](text.md) и [`TextInput`](textinput.md) внутри [`View`](view.md) ниже, и React Native отобразит их вместе:
...
Большинство основных компонентов React Native также могут быть настроены с помощью пропсов. Например, при использовании [`Image`](image.md) вы передаете ему пропс [`source`](image.md), чтобы определить, какое изображение он показывает:
...
У `Image` есть [множество различных пропсов](image.md), включая [`style`](image.md), который принимает JS-объект пар свойств-значений, связанных с дизайном и компоновкой.
...
Вы можете создать множество вещей с помощью пропсов и основных компонентов [`Text`](text.md), [`Image`](image.md) и [`View`](view.md)! Но чтобы создать что-то интерактивное, вам понадобится состояние.
...
!!!warning ""
    ...
    Видите `<>` и <code>&lt;/&gt;</code> выше? Эти кусочки JSX являются [фрагментами](https://reactdev.ru/reference/react/Fragment/). Смежные элементы JSX должны быть обернуты в окружающий тег. Фрагменты позволяют сделать это без вложения дополнительного, ненужного элемента обертки, такого как `View`.
