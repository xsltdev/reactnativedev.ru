...
Вы можете установить [`keyExtractor`]flatlist.md#keyextractor) для вашего компонента `FlatList`. Этот пропс используется для кэширования и как React `key` для отслеживания переупорядочивания элементов.
