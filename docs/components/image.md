...
В настоящее время поддерживаются следующие форматы: `png`, `jpg`, `jpeg`, `bmp`, `gif`, `webp`, `psd` (только для iOS). Кроме того, iOS поддерживает несколько форматов изображений RAW. Актуальный список поддерживаемых моделей камер см. в документации Apple (для iOS 12 см. https://support.apple.com/en-ca/HT208967).
